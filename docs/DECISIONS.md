...
> 本 skill 是 CKA 备考辅助工具，**不保证通过考试**。判卷基于本地 kind/k3s 环境，与真实考试环境（PSI 远程监考平台、特定 Kubernetes 版本、特定网络插件等）可能存在版本或行为差异，请以官方文档为准。本 skill **不提供、不复述任何真实考试题目**，所有练习题均为原创，仅用于训练同等能力点。本 skill 模拟"open book"规则，范围与真实考试一致：仅允许引用 `kubernetes.io/docs`、`kubernetes.io/blog`、`github.com/kubernetes`，不引用/不代做超出该范围的信息源。
